...
- **Remap preview** - Change in a key group or device popover targets that command's selected physical key across its game situations. Other keyboard, mouse, and gamepad keys stay untouched. The preview uses compact assignment counts and shows the selected old key before applying. The free-key assignment picker retains its whole-command replacement preview. Conflicts are re-scanned after applying.
...
- **Mappings list and conflicts** - one collapsible group per physical key shows a short action summary. Expand it for individual commands, sources, game situations, and key-scoped Change buttons. Different commands sharing a key remain separate controls; `IK_None` appears in a separate Unbound group. Search opens matching groups, and source/device filters limit the list. Clicking a conflict row switches to the matching device, focuses its key, and opens its details. Console-only debug bindings remain hidden.
- **Compact assignment model** - commands keep their canonical game/mod identity. Each `commands[].keys[]` assignment groups repeated rows by key, trigger state, and hold duration, carrying distinct `contexts`, `actions`, and `bindingCount`. Raw `commands[].bindings` are no longer returned. Original file text remains available for lossless editing; conflict detection still uses the full parser entries.
...
Validation: `npm test` covers compact assignment grouping (empty inputs, duplicate
rows, distinct hold durations, contexts and unbound ordering), old-key/section
remap isolation, missing matches, and malformed target keys. These checks use
synthetic input and do not write to the live game file. Browser interaction checks
are separate from the dependency-free automated suite.
