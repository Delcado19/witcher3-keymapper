...
- **Layout editor** - on the Mouse/Gamepad tab, "Edit layout" turns the scheme into a direct-manipulation editor: drag the PNG to move it, drag its corner handle to resize, drag any anchor dot (`ax`/`ay`) or any label (`lx`/`ly`), double-click a label to rename its button caption (RB/LT/X/A/START/R3/Back, etc.), and use A-/A+ to scale the label font (`fontSize`). Dragging snaps to a grid and to smart alignment guides against other controls' anchors/labels and the canvas/PNG centres (hold **Shift** to bypass snapping). "Save" persists the edited `profile.json` back to disk via `POST /api/profile` (backed up under `.cache/`); "Reset" reverts to the saved file.
