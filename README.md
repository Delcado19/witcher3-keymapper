...
- **Mappings list and conflicts** - the device diagram is the primary overview; a slim one-line summary (bindings, conflicts, commands, mod actions, syntax) sits above it and the conflicts list is a collapsible panel, since the diagram already glows the conflicted keys. Clicking a conflict row switches to the matching device, focuses the physical key, and opens its details; clicking a conflicting key still highlights the matching conflict row. Below the diagram, search and source/device filters drive a compact three-column command list (summary, keys, Change). Console-only debug bindings (`Debug_*`, `SCN_DBG_*`) are hidden from the list since they are not player keybindings.
