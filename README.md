...
- **Controller-scheme anchors** - mouse/gamepad control positions are defined per profile as `ax`/`ay` (a percentage of the device photo) plus a `side` (left/right column) in `public/devices/*/profile.json`, so the leader-line layout is data-driven and editable without touching code. An optional `leader` object per profile can override the global text-column width (`col`, or `colLeft`/`colRight` per side), label pitch (`minPitch`), and stacked-action cap (`maxLines`) - the mouse uses this to keep its photo prominent without clipping its long right-side labels.
